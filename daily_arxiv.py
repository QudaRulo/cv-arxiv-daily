...
def demo(**config):
    ...
    archive_root = config.get('archive_root', './archives')
    ...
    # 4. Archive old papers if enabled
    if enable_archive and publish_readme and not config['update_paper_links']:
        logging.info("Starting archive process...")
        try:
            from archive_manager import archive_papers
            archive_papers(
